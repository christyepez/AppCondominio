...
<div class="form-actions"><button class="button primary" type="submit" [disabled]="busy||!payable.supplierId">Registrar obligación</button></div></form></section>
...
export class TreasuryComponent implements OnInit{private readonly http=inject(HttpClient);private readonly communityOptions=inject(CommunityOptionsService);communities:CommunityOption[]=[];suppliers:TreasurySupplierOption[]=[];busy=false;error='';message='';forecast:unknown;forecastCommunityId='';payable={communityId:'',supplierId:'',documentNumber:'',documentDate:'',dueDate:'',amount:0,expenseAccount:'',payableAccount:''};
