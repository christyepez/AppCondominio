...
<label>Community Id<input [(ngModel)]="payable.communityId" name="communityId" required></label><label>Supplier Id<input [(ngModel)]="payable.supplierId" name="supplierId" required></label>
...
<section class="panel"><h3>Proyección de caja</h3><div class="form-grid compact"><label class="wide">Community Id<input [(ngModel)]="forecastCommunityId" name="forecastCommunityId"></label><div class="form-actions"><button class="button secondary" type="button" (click)="loadForecast()" [disabled]="busy||!forecastCommunityId">Consultar</button></div></div><pre class="result-box" *ngIf="forecast">{{forecast|json}}</pre></section>`})
export class TreasuryComponent{private readonly http=inject(HttpClient);busy=false;error='';message='';forecast:unknown;forecastCommunityId='';payable={communityId:'',supplierId:'',documentNumber:'',documentDate:'',dueDate:'',amount:0,expenseAccount:'',payableAccount:''};
