  ...
  template: `
    ...
    <section class="panel"><h3>Nueva cuenta bancaria</h3>
      <form class="form-grid" (ngSubmit)="createAccount()">
        <label>Comunidad<select [(ngModel)]="account.communityId" name="communityId" required><option value="">Seleccione una comunidad</option><option *ngFor="let community of communities" [value]="community.id">{{ community.name }} ({{ community.code }})</option></select></label>
...
export class BankingComponent implements OnInit {
  private readonly http=inject(HttpClient);private readonly communityOptions=inject(CommunityOptionsService);communities:CommunityOption[]=[]; busy=false; error=''; message='';
