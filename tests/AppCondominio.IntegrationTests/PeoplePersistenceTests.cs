...
public sealed class PeoplePersistenceTests:IAsyncLifetime
{
    ...
    public async Task Sprint06_persists_people_ownership_and_activation_lifecycle()
    {
        ...
        await using(var scope=provider.CreateAsyncScope())
        {
            ...
            Guid tenantId=await service.CreateNaturalPersonAsync(communityId,"TENANT-DEMO","Tenant Demo",null,null,null,null,default);
            ...
            grantId=await service.ApproveActivationAsync(activationId,"admin-demo",DateTimeOffset.UtcNow.AddMinutes(1),default);
