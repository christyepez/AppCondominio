...
public sealed class AccountingPersistenceTests:IAsyncLifetime
{
    ...
    public async Task Sprint11_posts_balanced_journal_and_generates_trial_balance()
    {
        ...
        await using(var scope=provider.CreateAsyncScope())
        {
            ...
            await s.ReverseAsync(journalId,"accountant",default);var tb2=await s.GetTrialBalanceAsync(communityId,default);Assert.Equal(2,tb2.Count);Assert.All(tb2,x=>Assert.Equal(0m,x.Balance));Assert.Equal(200m,tb2.Sum(x=>x.Debit));Assert.Equal(200m,tb2.Sum(x=>x.Credit));
        }
