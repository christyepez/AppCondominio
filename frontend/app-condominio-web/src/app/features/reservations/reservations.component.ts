...
<label>Community Id<input [(ngModel)]="area.communityId" name="communityId" required></label><label>Código<input [(ngModel)]="area.code" name="code" required></label>
...
<label>Area Id<input [(ngModel)]="booking.areaId" name="areaId" required></label><label>Requester Id<input [(ngModel)]="booking.requesterId" name="requesterId" required></label>
...
export class ReservationsComponent{private readonly http=inject(HttpClient);busy=false;error='';message='';area={communityId:'',code:'',name:'',capacity:1,fee:0};booking={areaId:'',requesterId:'',startsAt:'',endsAt:'',guests:0};
