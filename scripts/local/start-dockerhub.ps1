...
foreach ($key in @('APPCONDOMINIO_API_IMAGE','APPCONDOMINIO_WORKER_IMAGE','APPCONDOMINIO_WEB_IMAGE')) {
...
if (-not $SkipPull) { docker compose @files --env-file $EnvironmentFile pull appcondominio-api appcondominio-worker appcondominio-web; if ($LASTEXITCODE -ne 0) { throw 'Docker Hub image pull failed.' } }
