  ...
  <div class="supplier-grid" *ngIf="context">
   <section class="panel">
    ...
    <div class="round-list" *ngIf="rounds.length; else noRounds">
     <article class="round-card" *ngFor="let round of rounds" [class.selected]="round.id===selectedRoundId" (click)="selectedRoundId=round.id">
      <div><strong>{{ round.title }}</strong><small>Cierra: {{ round.closesAtUtc | date:'medium' }}</small></div><button type="button" class="button secondary" (click)="selectedRoundId=round.id; $event.stopPropagation()">Ofertar</button>
     </article>
