...
@Component({
  ...
  template: `
    <section class="page-heading">
      <div><span class="eyebrow">Administración</span><h2>Comunidades</h2><p>Gestión del maestro de conjuntos y condominios.</p></div>
    ...
    <section class="panel">
      ...
      <form class="form-grid" (ngSubmit)="create()">
        <label>Organization Id<input name="organizationId" [(ngModel)]="draft.organizationId" required placeholder="GUID de la organización"></label>
        <label>Código<input name="code" [(ngModel)]="draft.code" required maxlength="40"></label>
        ...
        <label class="wide">Dirección<input name="address" [(ngModel)]="draft.address" required maxlength="300"></label>
        <div class="form-actions"><button class="button primary" type="submit" [disabled]="saving">{{ saving ? 'Guardando…' : 'Crear comunidad' }}</button></div>
      </form>
    </section>
    ...
    <section class="panel">
      ...
      <div class="table-wrap" *ngIf="communities.length; else empty">
        <table>
          <thead><tr><th>Código</th><th>Nombre</th><th>RUC</th><th>Dirección</th><th>Estado</th><th>Id</th></tr></thead>
          <tbody><tr *ngFor="let item of communities"><td>{{ item.code }}</td><td>{{ item.name }}</td><td>{{ item.taxId || '—' }}</td><td>{{ item.address }}</td><td>{{ item.status }}</td><td class="mono">{{ item.id }}</td></tr></tbody>
        </table>
      </div>
...
export class CommunitiesComponent implements OnInit {
  ...
  private describeError(err: { status?: number }, fallback: string): string {
    if (err?.status === 401) return 'Sesión requerida. El JWT debe ser emitido por PortalCorporativo.';
