...
<label>Community Id<input [(ngModel)]="plan.communityId" name="communityId" required></label><label>Año<input [(ngModel)]="plan.year" name="year" type="number" required></label>
...
export class BudgetingComponent{private readonly http=inject(HttpClient);busy=false;error='';message='';variance:unknown;planId='';plan={communityId:'',year:new Date().getFullYear(),version:1,name:''};
